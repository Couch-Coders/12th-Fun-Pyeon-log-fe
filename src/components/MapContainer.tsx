...
const MapContainer: React.FC<MapPropsType> = ({ keyword }) => {
  ...
  useEffect(() => {
    if (mapApi) {
      ...
      // 키워드로 장소를 검색합니다
      const searchTerm = keyword ? `${keyword} 편의점` : '편의점'
      searchKeywordFunction(searchTerm, mapApi, mapApi.getCenter())
    }
  }, [mapApi, keyword])
  ...
  const searchKeywordFunction = (
    ...
    ps.keywordSearch(
      ...
      { category_group_code: 'CS2', location: locPosition }
    )
  }
  ...
  return (
    <MapCon>
      ...
      <Button onClick={moveToCenter}>내위치로 이동</Button>
    </MapCon>
  )
}
